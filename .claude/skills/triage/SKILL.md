...
anything the human needs to actually read before deciding — a codebase
summary, a full agent brief, the reasoning behind a category/state
recommendation — present it as prose or a document; a structured question
can't hold that much content.

This applies to step 2 ("Recommend") under "Triage a specific issue," where
the agent hands the maintainer a category/state recommendation, its
reasoning, and a codebase summary, then waits for direction.
...
2. **Recommend.** Tell the maintainer your category and state recommendation with reasoning, plus a brief codebase summary relevant to the issue — in plain, teachable language, bundled into one message, with an invitation to ask before they decide (see "Presenting decisions to the human" above). Wait for direction.
